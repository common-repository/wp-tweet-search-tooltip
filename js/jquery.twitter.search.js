...
;(function($) {
	...
	$.fn.twitterSearch.formatter = function(json, opts) {
		...
		s += '<div><span class="twitterSearchUser"><a target="_blank" href="http://www.twitter.com/'+ json.from_user+'/status/'+ json.id +'">'
	...
	$.fn.twitterSearch.defaults = {
		...
		css: {
			// default styling
			a:     { textDecoration: 'none', color: '#1485A5' },
			bird:  { width: '50px', height: '20px', position: 'absolute', left: '-30px', top: '-20px', border: 'none' },
			container: { overflow: 'hidden', backgroundColor: '#eee', height: '253px', border : '1px solid #ccc'},
			...
			frame: { height: '280px', border: '10px solid #C2CFF1', borderRadius: '10px', '-moz-border-radius': '10px', '-webkit-border-radius': '10px', '-moz-box-shadow': '1px 1px 5px #000', '-webkit-box-shadow': '1px 1px 5px #000' },
			tweet: { padding: '5px 10px', clear: 'left' },
			img:   { 'float': 'left', margin: '5px 10px 5px 0px', width: '48px', height: '48px','-moz-box-shadow': '1px 1px 5px #000', '-webkit-box-shadow': '1px 1px 5px #000'  },
			loading: { padding: '20px', textAlign: 'center', color: '#888', fontSize: '10px'},
			text:  { fontSize: '11px', borderBottom:'1px solid #ddd', paddingBottom:'6px' },
			time:  { fontSize: '10px', color: '#888' },
			title: { cursor: 'move', backgroundColor: '#C2CFF1', margin: 0, padding: '0 0 5px 0', textAlign: 'center', fontWeight: 'bold', fontSize: '16px', textShadow: '1px 1px 1px #fff',position: 'relative' },
			titleLink: { textDecoration: 'none', color: '#1485A5' },
			user:  { fontWeight: 'bold', fontSize:'11px', textTransform:'uppercase', textShadow:'1px 1px 1px #fff' }
		}
	};
